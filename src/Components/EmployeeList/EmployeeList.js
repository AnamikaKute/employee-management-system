...
export default function EmployeeList() {
    ...
    useEffect(() => {
        ...
        const deleteEmployee = (id) => {
            axios.delete("http://localhost:4200/employees/" + id)
                .then(() => {
                    axios.get("http://localhost:4200/employees")
                        .then((response) => {
                            setEmployees(response.data)
                    ...
                    return (
                        <tr key={singleemployee.id}>
                            ...
                            <td className=" border p-4 flex  gap-3 ">
                                <button onClick={()=>handleEdit (singleemployee.id)} className="bg-blue-600  text-white px-3 py-1 rounded-md">
                                    Edit
                                </button>
                                <button onClick={() => deleteEmployee(singleemployee.id)} className="bg-teal-600 text-white px-3 py-1 rounded-md">
